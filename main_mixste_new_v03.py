...
def load_backbone_checkpoint_from_path(path, model, device='cpu'):
    ...
    This lets you pass a FILE path (e.g. --previous_dir /path/to/model.pth),
    not just a directory. It is robust to different checkpoint formats:
      - plain state_dict
      - {'state_dict': ...}
      - {'model': ...} or {'model_pos': ...}
      - with or without 'module.' prefixes.
    """
    print(f"Loading backbone checkpoint from file: {path}")
    checkpoint = torch.load(path, map_location=device)

    # Figure out where the actual state dict is stored
    state_dict = None
    if isinstance(checkpoint, dict):
        if 'model' in checkpoint and isinstance(checkpoint['model'], dict):
            state_dict = checkpoint['model']
        elif 'model_pos' in checkpoint and isinstance(checkpoint['model_pos'], dict):
        ...
        # fallback: checkpoint is already a state_dict-like object
        state_dict = checkpoint
    ...
    try:
        ...
        print(f"Checkpoint loaded. Missing keys: {missing}, unexpected keys: {unexpected}")
...
def test(actions, dataloader, model):
    ...
    for i, data in enumerate(tqdm(dataloader, dynamic_ncols=True)):
        ...
        # input_2D: [B, 2, T, J, 2] (non-flip, flip)
...
if __name__ == '__main__':
    ...
    if args.previous_dir != '':
        if args.previous_dir.endswith('.pth'):
            ...
            # Original behavior: treat previous_dir as a DIRECTORY with checkpoints
